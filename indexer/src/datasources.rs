use {
    ...
    carbon_rpc_transaction_crawler_datasource::{
        ConnectionConfig, Filters as TransactionFilters, RetryConfig, RpcTransactionCrawler,
    },
    helius::types::{
    ...
    solana_client::{
        ...
        rpc_config::{RpcBlockConfig, RpcProgramAccountsConfig},
    },
...
/// Creates a configured Helius Atlas WebSocket datasource for Omnipair
...
pub fn create_helius_datasource(api_key: &str, program_id: Pubkey) -> HeliusWebsocket {
    ...
    let cluster = match std::env::var("CLUSTER")
        .unwrap_or_default()
        .to_lowercase()
        .as_str()
    {
...
/// This is used to convert START_SLOT to a before_signature for the transaction
/// crawler
async fn find_signature_at_slot(
    rpc_url: &str,
    program_id: Pubkey,
    target_slot: u64,
) -> Option<Signature> {
    ...
    log::info!(
        "Searching for signature at or before slot {} for program {}",
        target_slot,
        program_id
    );
    ...
    while !found_signature && attempts < MAX_ATTEMPTS {
            ...
            .await
        {
            Ok(signatures) => {
                ...
                log::info!(
                    "Fetched {} signatures, checking slots from {} to {}",
                    signatures.len(),
                ...
                for sig_info in &signatures {
                    if sig_info.slot <= target_slot {
                        log::info!(
                            "Found signature {} at slot {} (target: {})",
                            sig_info.signature,
                            sig_info.slot,
                            target_slot
                        );
                        return sig_info.signature.parse().ok();
                    }
                }
                ...
                if let Some(last_sig) = signatures.last() {
                    ...
                    log::info!(
                        "Continuing search from signature {} at slot {}",
                        last_sig.signature,
                        last_sig.slot
                    );
                } else {
            ...
            Err(e) => {
                log::warn!(
                    "Failed to fetch signatures for slot conversion (attempt {}): {:?}",
                    attempts,
                    e
                );
                break;
    ...
    if attempts >= MAX_ATTEMPTS {
        log::warn!(
            "Reached maximum attempts ({}) while searching for signature at slot {}",
            MAX_ATTEMPTS,
            target_slot
        );
    }
    ...
    log::warn!(
        "No signature found at or before slot {} for program {}",
        target_slot,
        program_id
    );
    None
...
/// Creates a configured RPC Transaction Crawler datasource for Omnipair
/// transaction monitoring This is more efficient than the block crawler as it
/// pre-filters transactions by program ID
pub async fn create_transaction_crawler_datasource(
    htt_rpc_url: String,
    program_id: Pubkey,
    start_block: Option<u64>,
) -> CarbonResult<RpcTransactionCrawler> {
    let connection_config = ConnectionConfig::new(
        ...
        5,                      /* max_concurrent_requests: fetch up to 5 transactions
                                 * concurrently */
    ...
    // We use until_signature because the crawler processes newest first, and we
    // want to start from START_BLOCK
    let until_signature = if let Some(block_number) = start_block {
        if block_number == 0 {
            ...
            log::info!(
                "Converting START_BLOCK {} to slot and signature for transaction crawler",
                block_number
            );
            ...
            match get_slot_for_block(&htt_rpc_url, block_number).await {
                Some(slot) => {
                    ...
                    match find_signature_at_slot(&htt_rpc_url, program_id, slot).await {
                        Some(sig) => {
                            log::info!(
                                "Found signature {} at slot {} (block {}) or earlier",
                                sig,
                                slot,
                                block_number
                            );
                ...
                None => {
                    log::warn!(
                        "Failed to find slot for block {}, starting from most recent",
                        block_number
                    );
                    None
                }
    ...
    let filters = TransactionFilters::new(
        ...
        until_signature, /* until_signature: stop at START_BLOCK signature (process from
               * START_BLOCK to current) */
    );

    // Log the final filter configuration
    match &until_signature {
        Some(sig) => log::info!(
            "Transaction crawler configured to process from signature: {} to current",
            sig
        ),
...
impl Datasource for GpaBackfillDatasource {
    async fn consume(
        ...
        let slot = rpc_client.get_slot().await.map_err(|_| {
            carbon_core::error::Error::FailedToConsumeDatasource("Failed to fetch slot".to_string())
        })?;
        ...
        log::info!(
            "Found {} program accounts for backfill",
