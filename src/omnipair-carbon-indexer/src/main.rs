use {
    ...
    carbon_omnipair_decoder::{
        accounts::OmnipairAccount,
        instructions::{OmnipairInstruction, OmnipairInstructionType},
        OmnipairDecoder, PROGRAM_ID as OMNIPAIR_PROGRAM_ID,
    },
...
pub struct OmnipairAccountProcessor;

#[async_trait]
impl Processor for OmnipairAccountProcessor {
    ...
    async fn process(
        ...
        match account.data {
            ...
            OmnipairAccount::UserPosition(user_position) => {
                log::info!("UserPosition: {:?}", &user_position);
            }
            _ => {}
        }
...
instruction_decoder_collection!(
    ...
    Omnipair => OmnipairDecoder => OmnipairInstruction
);
...
async fn main() -> anyhow::Result<()> {
    ...
    Pipeline::builder()
        ...
        .account(OmnipairDecoder, OmnipairAccountProcessor)
